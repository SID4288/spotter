...
2. **December 2025 Fixed-Lane Forecast**: 31 daily predictions for the fixed corridor **Lexington, KY $\rightarrow$ Fort Wayne, IN** (360 miles, Dry Van, 32,000 lb) reflecting weekly dynamics and quarter-end volume surges (`output/december_predictions.csv`).
...
* **Distance-Aware Corruption Cleaning**: Identified and removed ~1.41% (677 rows) synthetic label perturbations in training data via polynomial log-distance MAD residuals ($r = \log(\text{rate}) - \text{polyfit}(\log(\text{dist})) > 6 \cdot \text{MAD}$). Eliminating these corrupted labels prevents tree ensembles from splitting on noisy artifacts.
* **Log-Target Transformation**: Models trained on $\log(\text{posted\_rate})$ to stabilize multiplicative variance across route lengths and directly minimize percentage error (MAPE).
* **Extrapolation-Ready Temporal Seasonality**: Replaced raw calendar fields (`month`, `quarter`, `week_of_year`) that fail on out-of-sample future dates with `days_to_quarter_end`, `day_of_week`, `day_of_month`, and `is_weekend` to capture cyclical volume surges.
* **Multi-Fold Forward-Chaining Validation**: Evaluated across 3 rolling 2-month horizons (predicting 61 days forward), measuring both clean-label and all-row metrics.
* **Winning Model**: **`ExtraTreesRegressor (max_features='sqrt')`** proved most robust, achieving **2.72% Clean MAPE** on out-of-sample forward folds.

### Model Benchmark (Fold 2: Aug–Sep 61-Day Forecast):
